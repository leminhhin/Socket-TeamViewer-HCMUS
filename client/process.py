...
class process_frame(tk.Frame):
	...
	def make_widgets(self):
		...
		self.view_button = thm.Button(frame2, text = "Xem", command=self.get_running_processes)
	...
	def get_running_processes(self):
		try:
			procs = self.tmp_client.req_process_getall(self.tmp_client)
	...
	def kill_window(self):
		...
		self.top.geometry("582x50+100+100")
	...
	def kill_process(self):
		...
		if res:
			msbx.showinfo('', "Đã diệt Process")
		else:
			msbx.showerror('', "Không tìm thấy Process")
	...
	def start_process(self):
		...
		if res:
			msbx.showinfo('', "Process đã được bật")
		else:
			msbx.showerror('', "Không tìm thấy process")
